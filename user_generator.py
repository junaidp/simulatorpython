    ...
    def generate_historical_events(users, days=180, platforms=['slack', 'teams', 'jira']):
        ...
        while current_date < end_date:
            if current_date.weekday() < 5:  # Weekdays only
                for user in users:
                    # Platform distribution: Slack 60%, Teams 25%, Jira 15%
                    platform_weights = [0.60, 0.25, 0.15]

                    for platform in platforms:
                        if platform == 'slack':
                            weight = platform_weights[0]
                        elif platform == 'teams':
                            weight = platform_weights[1]
                        else:
                            weight = platform_weights[2]

                        # Calculate events for this platform
                        daily_events = EventGenerator.calculate_daily_events(user, platform)
                        daily_events = int(daily_events * weight)
                        ...
                        for _ in range(daily_events):
                            ...
                            event = EventGenerator.generate_event(user, platform, event_time, source='historical')
                            events.append(event)
